...
function assignButtonActiveClass(dayIndex) {
  ...
  btn[dayIndex + 1].classList.add("active");
...
function populateTimetableDropdown() {
  ...
  const endTime = new Date("1970-01-01T11:00:00");

  while (currentTime <= endTime) {
    allTimes.push(currentTime.toTimeString().slice(0, 5));
    currentTime = new Date(currentTime.getTime() + 10 * 60 * 1000);
  }

  if (dateIndexInDays !== -1 && dayData[dateIndexInDays]) {
    const takenTimes = dayData[dateIndexInDays].map((student) => student.time);
